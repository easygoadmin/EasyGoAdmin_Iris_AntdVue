...
func (c *ItemController) Detail(ctx iris.Context) {
	// 记录ID
	...
	if id > 0 {
		...
		// 属性拷贝
		var itemInfoVo = &vo.ItemInfoVo{}
		utils.StructCopy(info, itemInfoVo)

		// 站点图片
		if info.Image != "" {
			itemInfoVo.Image = utils.GetImageUrl(info.Image)
		}
