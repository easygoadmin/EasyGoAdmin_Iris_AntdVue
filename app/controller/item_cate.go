...
func (c *ItemCateController) Detail(ctx iris.Context) {
	// 记录ID
	...
	if id > 0 {
		...
		// 属性拷贝
		var itemCateInfoVo = &vo.ItemCateInfoVo{}
		utils.StructCopy(info, itemCateInfoVo)

		// 站点封面
		if info.Cover != "" {
			itemCateInfoVo.Cover = utils.GetImageUrl(info.Cover)
		}
