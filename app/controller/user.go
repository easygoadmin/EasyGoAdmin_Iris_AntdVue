...
func (c *UserController) Detail(ctx iris.Context) {
	// 记录ID
...
func (c *UserController) CheckUser(ctx iris.Context) {
	// 用户名
	userName := ctx.Params().GetString("username")
	// 调用检查用户方法
	user, err := service.User.CheckUser(userName)
	if err != nil {
		ctx.JSON(common.JsonResult{
			Code: -1,
			Msg:  err.Error(),
